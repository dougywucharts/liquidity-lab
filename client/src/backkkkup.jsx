...
const styles = {
  ...
  mainGrid: {
    ...
    alignItems: "start", // 🔥 THIS FIXES THE GAP
    gap: 16,
  },
  panel: {
    borderRadius: 22,
    border: `1px solid ${palette.border}`,
    background:
      "linear-gradient(180deg, rgba(10,14,22,0.98), rgba(6,10,16,0.98))", // 🔥 slightly darker top
...
function ChartLevelOverlay({ event }) {
  ...
  const entryPct = entry ? 14 + ((high - entry.value) / range) * 72 : null;
  const stopPct = stop ? 14 + ((high - stop.value) / range) * 72 : null;
  ...
  const tpPct = tp1 ? 14 + ((high - tp1.value) / range) * 72 : null;
  ...
  return (
    ...
    >
      ...
      {levels.map((level) => {
        const pct = 14 + ((high - level.value) / range) * 72;
        const color =
          level.tone === "long"
        ...
        <div style={styles.mainGrid}>
          ...
          >
            ...
            <div
              style={{
                ...styles.panel,
                marginTop: -6, // 🔥 pulls it into the chart slightly
              }}
            >
              <div
                style={{
