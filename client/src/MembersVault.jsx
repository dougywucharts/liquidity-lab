...
const PATTERNS = [
  {
    ...
    entry: "Short on the first 1m candle that closes back below the swept level, after a rejection wick forms.",
  ...
  },
  {
    ...
    entry: "Long on the first 1m candle that closes back above the swept level with a bullish body and rejection wick.",
  ...
  },
  {
    ...
    entry: "On the reclaim of the level after the second sweep. Treat it like a High or Low Sweep entry but with higher conviction.",
  ...
  },
  {
    ...
    tone: "short",
    icon: "══↑",
    tagline: "Two or more highs at the same level = a stop cluster waiting to be raided.",
    ...
    entry: "Short after the sweep and rejection, same as a High Sweep. The equal highs label tells you WHY the level matters, not a different entry technique.",
  ...
  },
  {
    ...
    tone: "long",
    icon: "══↓",
    tagline: "Two or more lows at the same level = a stop cluster waiting to be swept long.",
    ...
    entry: "Long after the sweep and reclaim, same mechanics as a Low Sweep entry.",
  ...
  },
  {
    ...
    tone: "short",
    icon: "~✕",
    tagline: "Price taps the EMA99 and gets slapped down. The trend accelerates.",
    ...
    entry: "Short on the 1m rejection candle from the EMA99. Combine with a sweep of a local high for maximum conviction.",
  ...
  },
  {
    ...
    tone: "short",
    icon: "↑✗",
    tagline: "Price tries to reclaim a swept level and fails. Continuation lower.",
    ...
    entry: "Short on the candle that fails to close above the level on the reclaim attempt. The failed reclaim IS the entry signal.",
  ...
  },
  {
    ...
    entry: "The displacement candle itself is often too late for a clean entry. Look for a pullback to the reclaim level or the 50% of the displacement candle.",
  ...
  },
  {
    ...
    entry: "Market order on the close of the confirmation candle. Accept the slightly worse fill as the cost of certainty.",
...
function Pill({ children, tone = "neutral", small }) {
  const map = {
    neutral: { color: "rgba(244,247,251,0.66)", bg: "rgba(255,255,255,0.05)", border: "rgba(255,255,255,0.08)" },
    long:    { color: palette.long, bg: palette.longSoft, border: "rgba(74,222,128,0.26)" },
    short:   { color: palette.short, bg: palette.shortSoft, border: "rgba(251,113,133,0.26)" },
    gold:    { color: palette.gold, bg: palette.goldSoft, border: "rgba(246,196,83,0.24)" },
    blue:    { color: palette.blue, bg: palette.blueSoft, border: "rgba(96,165,250,0.24)" },
  };
  const s = map[tone] || map.neutral;
  return (
    <span style={{ display:"inline-flex", alignItems:"center", padding:small?"2px 7px":"4px 10px", borderRadius:999, fontSize:small?10:11, fontWeight:800, letterSpacing:0.3, color:s.color, background:s.bg, border:`1px solid ${s.border}` }}>
      {children}
    </span>
  );
}
...
function PatternCard({ pattern, onClick }) {
  ...
  return (
    <button
      ...
      style={{ appearance:"none", WebkitAppearance:"none", border:`1px solid ${hovered ? pattern.color.replace(")", ",0.4)").replace("rgb","rgba") : "rgba(255,255,255,0.08)"}`, background: hovered ? pattern.bg : "linear-gradient(180deg,rgba(15,20,32,0.96),rgba(10,14,24,0.96))", borderRadius:18, padding:"20px 22px", cursor:"pointer", textAlign:"left", color:palette.text, display:"grid", gap:12, transition:"all 0.2s ease", transform:hovered?"translateY(-2px)":"translateY(0)", boxShadow:hovered?`0 8px 24px ${pattern.bg}`:"none" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
        <div style={{ fontSize:28, fontWeight:900, fontFamily:"monospace", color:pattern.color, opacity:0.8 }}>{pattern.icon}</div>
        <Pill tone={pattern.tone} small>{pattern.tag}</Pill>
      </div>
      <div>
        <div style={{ fontSize:16, fontWeight:900, marginBottom:6 }}>{pattern.name}</div>
        <div style={{ fontSize:12, color:palette.textSoft, lineHeight:1.5 }}>{pattern.tagline}</div>
      </div>
      <div style={{ fontSize:11, color:pattern.color, fontWeight:700, display:"flex", alignItems:"center", gap:4 }}>
        Read pattern guide →
      </div>
    </button>
  );
...
function PatternDetail({ pattern, onBack }) {
  return (
    <div style={{ display:"grid", gap:16, maxWidth:860, margin:"0 auto" }}>
      <button onClick={onBack} style={{ appearance:"none", border:"none", background:"none", color:palette.textDim, cursor:"pointer", fontSize:13, display:"flex", alignItems:"center", gap:6, padding:0, width:"fit-content" }}>
        ← Back to patterns
      </button>

      {/* Header */}
      <div style={{ borderRadius:22, border:`1px solid ${pattern.color}33`, background:`linear-gradient(135deg, ${pattern.bg}, rgba(5,8,14,0.98))`, padding:"28px 32px", display:"grid", gap:10 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
          <span style={{ fontSize:36, fontFamily:"monospace", color:pattern.color }}>{pattern.icon}</span>
          <div>
            <div style={{ fontSize:10, color:palette.textDim, letterSpacing:2, textTransform:"uppercase", marginBottom:4 }}>Pattern Guide</div>
            <div style={{ fontSize:28, fontWeight:900 }}>{pattern.name}</div>
          </div>
          <Pill tone={pattern.tone}>{pattern.tag}</Pill>
        </div>
        <div style={{ fontSize:15, color:palette.textSoft, lineHeight:1.6, maxWidth:600 }}>{pattern.tagline}</div>
      </div>

      {/* What it is */}
      <Section title="What it looks like" accent={pattern.color}>
        <p style={{ color:palette.textSoft, lineHeight:1.7, fontSize:14 }}>{pattern.what}</p>
      </Section>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12 }}>
        ...
        <LevelCard label="TP1 / TP2" color={palette.long} content={`${pattern.tp1}\n\n${pattern.tp2}`} />
      </div>
      ...
      <Section title="Common mistakes" accent={palette.short}>
        <div style={{ display:"grid", gap:8 }}>
          {pattern.mistakes.map((m, i) => (
            <div key={i} style={{ display:"flex", gap:10, alignItems:"flex-start", fontSize:13, color:palette.textSoft, lineHeight:1.5 }}>
              <span style={{ color:palette.short, fontSize:16, marginTop:1, flexShrink:0 }}>✗</span>
              <span>{m}</span>
            </div>
          ))}
        </div>
...
function Section({ title, accent, children }) {
  return (
    <div style={{ border:`1px solid ${palette.border}`, background:"linear-gradient(180deg,rgba(15,20,32,0.96),rgba(10,14,24,0.96))", borderRadius:18, padding:"20px 22px", display:"grid", gap:12 }}>
      <div style={{ fontSize:10, fontWeight:900, letterSpacing:2, textTransform:"uppercase", color:accent||palette.textDim }}>{title}</div>
      {children}
    </div>
  );
}
...
function LevelCard({ label, color, content }) {
  return (
    <div style={{ border:`1px solid ${color}33`, background:`${color}0d`, borderRadius:14, padding:"14px 16px", display:"grid", gap:8 }}>
      <div style={{ fontSize:10, fontWeight:900, letterSpacing:1.5, textTransform:"uppercase", color }}>{label}</div>
      <div style={{ fontSize:12, color:palette.textSoft, lineHeight:1.6, whiteSpace:"pre-line" }}>{content}</div>
    </div>
  );
}
...
function ExampleCard({ type, content }) {
  ...
  const icon = isGood ? "✓" : "✗";
  const label = isGood ? "What a good setup looks like" : "What a bad setup looks like";
  return (
    <div style={{ border:`1px solid ${color}33`, background:`${color}0d`, borderRadius:14, padding:"14px 16px", display:"grid", gap:8 }}>
      ...
      <div style={{ fontSize:12, color:palette.textSoft, lineHeight:1.6 }}>{content}</div>
    </div>
  );
}
...
function ArticleCard({ article, onClick }) {
  ...
  return (
    <button
      ...
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      style={{ appearance:"none", WebkitAppearance:"none", border:`1px solid ${hovered?"rgba(255,255,255,0.14)":palette.border}`, background:hovered?"rgba(255,255,255,0.04)":"linear-gradient(180deg,rgba(15,20,32,0.96),rgba(10,14,24,0.96))", borderRadius:16, padding:"18px 20px", cursor:"pointer", textAlign:"left", color:palette.text, display:"grid", gap:10, transition:"all 0.15s ease", transform:hovered?"translateY(-1px)":"translateY(0)" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8 }}>
        <Pill tone="neutral" small>{article.category}</Pill>
        <span style={{ fontSize:11, color:palette.textDim, fontWeight:600 }}>{article.readTime} read</span>
      </div>
      <div style={{ fontSize:15, fontWeight:800, lineHeight:1.4 }}>{article.title}</div>
      <div style={{ fontSize:12, color:palette.textSoft, lineHeight:1.5 }}>
        {article.body.split('\n')[0].slice(0, 100)}…
      </div>
      <div style={{ fontSize:11, color:article.color, fontWeight:700 }}>Read →</div>
    </button>
  );
}
...
function ArticleDetail({ article, onBack }) {
  ...
  const formatted = article.body.split('\n').map((line, i) => {
    ...
    return (
      <p key={i} style={{ margin:"0 0 4px", lineHeight:1.75, fontSize:14, color:line.startsWith('**') ? palette.text : palette.textSoft }}>
        {parts.map((part, j) =>
          part.startsWith('**') && part.endsWith('**')
            ? <strong key={j} style={{ color:palette.text, fontWeight:800 }}>{part.slice(2,-2)}</strong>
            : part
        )}
      </p>
    );
  ...
  return (
    <div style={{ maxWidth:680, margin:"0 auto", display:"grid", gap:16 }}>
      ...
      <div style={{ borderLeft:`3px solid ${article.color}`, paddingLeft:20, display:"grid", gap:8 }}>
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          <Pill tone="neutral" small>{article.category}</Pill>
          <span style={{ fontSize:11, color:palette.textDim }}>{article.readTime} read</span>
        </div>
        <div style={{ fontSize:26, fontWeight:900, lineHeight:1.25 }}>{article.title}</div>
      </div>

      <div style={{ border:`1px solid ${palette.border}`, background:"linear-gradient(180deg,rgba(15,20,32,0.96),rgba(10,14,24,0.96))", borderRadius:18, padding:"24px 28px", display:"grid", gap:4 }}>
        {formatted}
      </div>
    </div>
  );
...
function DnaStatBar({ label, value, max = 100, color }) {
  ...
  return (
    <div style={{ display:"grid", gap:6 }}>
      <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:palette.textSoft }}>
        <span>{label}</span>
...
function DnaCard({ label, value, subtext, accent, icon }) {
  return (
    <div style={{ border:`1px solid ${accent}33`, background:`${accent}0d`, borderRadius:16, padding:"16px 18px", display:"grid", gap:6 }}>
      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
        {icon && <span style={{ fontSize:18 }}>{icon}</span>}
        <div style={{ fontSize:10, fontWeight:900, letterSpacing:1.5, textTransform:"uppercase", color:accent }}>{label}</div>
      </div>
      <div style={{ fontSize:15, fontWeight:800, color:palette.text, lineHeight:1.4 }}>{value || "—"}</div>
      {subtext && <div style={{ fontSize:12, color:palette.textSoft, lineHeight:1.5 }}>{subtext}</div>}
    </div>
  );
}

function TraderDnaPage({ dna, setDna, loading, setLoading, error, setError, currentUser }) {
  ...
  return (
    <div style={{ display:"grid", gap:24, maxWidth:900, margin:"0 auto" }}>
      ...
      <div style={{ display:"grid", gap:8 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
          <div style={{ fontSize:36, fontWeight:900 }}>
            Trader DNA
            <sup style={{ fontSize:14, color:palette.gold, fontWeight:900, marginLeft:3 }}>™</sup>
          </div>
          <span style={{ padding:"4px 10px", borderRadius:999, background:"rgba(96,165,250,0.12)", border:"1px solid rgba(96,165,250,0.24)", fontSize:11, fontWeight:800, color:palette.blue }}>
            Claude-Reviewed™
          </span>
        </div>
        <div style={{ fontSize:14, color:palette.textSoft, maxWidth:540, lineHeight:1.6 }}>
          AI synthesizes every trade you've logged into a personal performance profile. Gets smarter as you log more trades. Requires 10+ trades.
        </div>
      </div>

      {/* Generate button or results */}
      {!dna && !loading && (
        <div style={{ border:`1px solid ${palette.border}`, borderRadius:20, padding:"40px 32px", textAlign:"center", display:"grid", gap:16, background:"linear-gradient(180deg,rgba(15,20,32,0.96),rgba(10,14,24,0.96))" }}>
          <div style={{ fontSize:56 }}>🧬</div>
          <div style={{ fontSize:20, fontWeight:900 }}>Generate Your DNA Profile</div>
          <div style={{ fontSize:13, color:palette.textSoft, maxWidth:400, margin:"0 auto", lineHeight:1.6 }}>
            Claude will analyze all your logged trades and build a detailed psychological and performance profile unique to you.
          </div>
          {error && (
            <div style={{ padding:"10px 16px", borderRadius:12, background:"rgba(251,113,133,0.1)", border:"1px solid rgba(251,113,133,0.3)", color:palette.short, fontSize:13, fontWeight:700 }}>
              {error}
            </div>
          )}
          <button onClick={generate}
            style={{ appearance:"none", border:"none", borderRadius:14, padding:"14px 32px", background:"linear-gradient(135deg,#f6c453,#d97706)", color:"#000", fontWeight:900, fontSize:15, cursor:"pointer", margin:"0 auto", letterSpacing:0.3 }}>
            🧬 Generate Trader DNA
          </button>
        </div>
      )}
      ...
      {loading && (
        <div style={{ border:`1px solid ${palette.border}`, borderRadius:20, padding:"60px 32px", textAlign:"center", display:"grid", gap:16, background:"linear-gradient(180deg,rgba(15,20,32,0.96),rgba(10,14,24,0.96))" }}>
          <div style={{ fontSize:48, animation:"spin 2s linear infinite" }}>🧬</div>
          <div style={{ fontSize:18, fontWeight:900 }}>Analyzing your trades…</div>
          <div style={{ fontSize:13, color:palette.textSoft }}>Claude is reading your trade history and building your profile.</div>
      ...
      {dna && !loading && (
        <div style={{ display:"grid", gap:16 }}>
          ...
          <div style={{ borderRadius:22, border:`1px solid rgba(246,196,83,0.3)`, background:"linear-gradient(135deg,rgba(246,196,83,0.08),rgba(5,8,14,0.98))", padding:"28px 32px", display:"grid", gap:14 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:12 }}>
              <div>
                <div style={{ fontSize:11, color:palette.gold, letterSpacing:2, textTransform:"uppercase", fontWeight:800, marginBottom:6 }}>Your Trader Type</div>
                <div style={{ fontSize:26, fontWeight:900 }}>{dna.traderType || "—"}</div>
              </div>
              <div style={{ display:"grid", gap:6, textAlign:"right" }}>
                <div style={{ fontSize:11, color:palette.textDim, letterSpacing:1, textTransform:"uppercase" }}>Risk Discipline</div>
                <div style={{ fontSize:32, fontWeight:900, color: dna.riskDisciplineScore >= 70 ? palette.long : dna.riskDisciplineScore >= 40 ? palette.gold : palette.short }}>
                  {dna.riskDisciplineScore ?? "—"}<span style={{ fontSize:16, color:palette.textDim }}>/100</span>
                </div>
              </div>
            </div>
            <div style={{ fontSize:14, color:palette.textSoft, lineHeight:1.7, borderTop:`1px solid ${palette.borderSoft}`, paddingTop:14 }}>
              {dna.overallAssessment}
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12 }}>
              <div style={{ textAlign:"center", padding:"10px", borderRadius:12, background:"rgba(255,255,255,0.03)" }}>
                <div style={{ fontSize:10, color:palette.textDim, textTransform:"uppercase", letterSpacing:1 }}>Trades</div>
                <div style={{ fontSize:22, fontWeight:900, marginTop:4 }}>{dna.totalTrades ?? "—"}</div>
              </div>
              <div style={{ textAlign:"center", padding:"10px", borderRadius:12, background:"rgba(255,255,255,0.03)" }}>
                <div style={{ fontSize:10, color:palette.textDim, textTransform:"uppercase", letterSpacing:1 }}>Win Rate</div>
                <div style={{ fontSize:22, fontWeight:900, color:palette.long, marginTop:4 }}>{dna.winRate != null ? `${Math.round(dna.winRate * 100)}%` : "—"}</div>
              </div>
              <div style={{ textAlign:"center", padding:"10px", borderRadius:12, background:"rgba(255,255,255,0.03)" }}>
                <div style={{ fontSize:10, color:palette.textDim, textTransform:"uppercase", letterSpacing:1 }}>Avg RR</div>
                <div style={{ fontSize:22, fontWeight:900, color:palette.blue, marginTop:4 }}>{dna.avgRR != null ? `${Number(dna.avgRR).toFixed(2)}R` : "—"}</div>
              </div>
            </div>
            <DnaStatBar label="Risk Discipline Score" value={dna.riskDisciplineScore ?? 0} color={dna.riskDisciplineScore >= 70 ? palette.long : dna.riskDisciplineScore >= 40 ? palette.gold : palette.short} />
          </div>

          {/* Strengths + Weaknesses */}
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            <div style={{ border:`1px solid rgba(74,222,128,0.2)`, background:"rgba(74,222,128,0.04)", borderRadius:18, padding:"20px 22px", display:"grid", gap:12 }}>
              <div style={{ fontSize:10, fontWeight:900, letterSpacing:2, textTransform:"uppercase", color:palette.long }}>Strengths</div>
              {(dna.strengths||[]).map((s,i) => (
                <div key={i} style={{ display:"flex", gap:10, alignItems:"flex-start", fontSize:13, color:palette.textSoft, lineHeight:1.5 }}>
                  <span style={{ color:palette.long, fontSize:16, flexShrink:0 }}>✓</span><span>{s}</span>
                </div>
              ))}
            </div>
            <div style={{ border:`1px solid rgba(251,113,133,0.2)`, background:"rgba(251,113,133,0.04)", borderRadius:18, padding:"20px 22px", display:"grid", gap:12 }}>
              <div style={{ fontSize:10, fontWeight:900, letterSpacing:2, textTransform:"uppercase", color:palette.short }}>Weaknesses</div>
              {(dna.weaknesses||[]).map((w,i) => (
                <div key={i} style={{ display:"flex", gap:10, alignItems:"flex-start", fontSize:13, color:palette.textSoft, lineHeight:1.5 }}>
                  <span style={{ color:palette.short, fontSize:16, flexShrink:0 }}>✗</span><span>{w}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:14 }}>
            <DnaCard label="Best Session" value={dna.bestSession} icon="🟢" accent="#4ade80" />
            <DnaCard label="Worst Session" value={dna.worstSession} icon="🔴" accent="#fb7185" />
            <DnaCard label="Best Setup" value={dna.bestSetup} icon="⭐" accent="#f6c453" />
            <DnaCard label="Worst Setup" value={dna.worstSetup} icon="⚠️" accent="#f87171" />
          </div>

          {/* Pattern bias + coaching focus */}
          <div style={{ border:`1px solid ${palette.border}`, borderRadius:18, padding:"20px 22px", display:"grid", gap:14, background:"linear-gradient(180deg,rgba(15,20,32,0.96),rgba(10,14,24,0.96))" }}>
            <div>
              <div style={{ fontSize:10, fontWeight:900, letterSpacing:2, textTransform:"uppercase", color:palette.textDim, marginBottom:8 }}>Pattern Bias</div>
              <div style={{ fontSize:14, color:palette.textSoft, lineHeight:1.6 }}>{dna.patternBias || "—"}</div>
            </div>
            <div style={{ borderTop:`1px solid ${palette.borderSoft}`, paddingTop:14 }}>
              <div style={{ fontSize:10, fontWeight:900, letterSpacing:2, textTransform:"uppercase", color:palette.gold, marginBottom:8 }}>🎯 Coaching Focus This Week</div>
              <div style={{ fontSize:15, color:palette.text, lineHeight:1.6, fontWeight:700 }}>{dna.coachingFocus || "—"}</div>
            </div>
          </div>
          ...
          <div style={{ display:"flex", justifyContent:"center", gap:12, paddingTop:8 }}>
            <button onClick={generate}
              style={{ appearance:"none", border:`1px solid ${palette.border}`, borderRadius:12, padding:"10px 24px", background:"rgba(255,255,255,0.04)", color:palette.textSoft, fontWeight:700, fontSize:13, cursor:"pointer" }}>
              🔄 Regenerate Profile
            </button>
          </div>
        </div>
...
export default function MembersVault({ onBack, currentUser, featureFlags }) {
  ...
  const isPro = ["active","trialing","beta","pro"].includes(currentUser?.stripeStatus||"") || featureFlags?.aiReview;

  const patternTones = ["all","short","long","gold","neutral"];
  const articleCategories = ["all","Risk","Psychology","Execution","Mindset","Management"];

  const filteredPatterns = patternFilter === "all" ? PATTERNS : PATTERNS.filter(p => p.tone === patternFilter);
  const filteredArticles = articleFilter === "all" ? PSYCH_ARTICLES : PSYCH_ARTICLES.filter(a => a.category === articleFilter);

  if (!isPro) {
    return (
      <div style={{ minHeight:"100vh", background:`radial-gradient(circle at 15% 20%, rgba(239,68,68,0.08), transparent 40%), linear-gradient(180deg,#060a12,#03060b)`, display:"grid", placeItems:"center", fontFamily:'Inter, ui-sans-serif, sans-serif', color:palette.text }}>
        <div style={{ textAlign:"center", display:"grid", gap:20, padding:32, maxWidth:420 }}>
          <div style={{ fontSize:48 }}>🔒</div>
          <div style={{ fontSize:10, color:palette.textDim, letterSpacing:3, textTransform:"uppercase" }}>Members Vault</div>
          <div style={{ fontSize:28, fontWeight:900 }}>Pro Access Only</div>
          <div style={{ fontSize:14, color:palette.textSoft, lineHeight:1.6 }}>The Members Vault is available on Pro plans. Upgrade to unlock the Pattern Library, Psychology Vault, and everything coming next.</div>
          <button onClick={onBack} style={{ border:`1px solid ${palette.border}`, borderRadius:14, padding:"11px 20px", background:"linear-gradient(180deg,rgba(20,27,42,0.96),rgba(12,17,28,0.96))", color:palette.text, cursor:"pointer", fontWeight:800, fontSize:14 }}>← Back to Dashboard</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight:"100vh", background:`radial-gradient(circle at 15% 20%, rgba(239,68,68,0.08), transparent 40%), linear-gradient(180deg,#060a12,#03060b)`, fontFamily:'Inter, ui-sans-serif, sans-serif', color:palette.text }}>

      {/* Top nav */}
      <div style={{ position:"sticky", top:0, zIndex:100, borderBottom:`1px solid ${palette.border}`, background:"rgba(3,6,11,0.92)", backdropFilter:"blur(12px)", padding:"0 24px", display:"flex", alignItems:"center", gap:0, height:54 }}>
        <button onClick={onBack} style={{ appearance:"none", border:"none", background:"none", color:palette.textDim, cursor:"pointer", fontSize:13, fontWeight:700, display:"flex", alignItems:"center", gap:6, padding:"0 16px 0 0", borderRight:`1px solid ${palette.border}`, height:"100%", whiteSpace:"nowrap" }}>
          ← Dashboard
        </button>
        <div style={{ padding:"0 16px", borderRight:`1px solid ${palette.border}`, height:"100%", display:"flex", alignItems:"center", gap:8 }}>
          <span style={{ fontSize:10, color:palette.textDim, letterSpacing:2, textTransform:"uppercase", fontWeight:800 }}>Members Vault</span>
          <span style={{ fontSize:10, padding:"2px 6px", borderRadius:4, background:palette.goldSoft, color:palette.gold, fontWeight:800 }}>PRO</span>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:2, padding:"0 12px", flex:1 }}>
          {[
            { id:"home", label:"Home" },
      ...
      { id:"videos", label:"Video Library" },
            ...
            <button key={tab.id} onClick={() => { setActiveSection(tab.id); setSelectedPattern(null); setSelectedArticle(null); }}
              style={{ appearance:"none", border:"none", background:"none", color: tab.gold && activeSection===tab.id ? palette.gold : activeSection===tab.id ? palette.text : palette.textDim, cursor:"pointer", fontSize:13, fontWeight:activeSection===tab.id?800:600, padding:"0 14px", height:"100%", borderBottom:activeSection===tab.id?`2px solid ${tab.gold?palette.gold:palette.accent}`:"2px solid transparent", transition:"all 0.15s ease", display:"flex", alignItems:"center", gap:4 }}>
              {tab.label}
              {tab.badge && <span style={{ fontSize:9, fontWeight:900, color:palette.gold, verticalAlign:"super", lineHeight:1 }}>{tab.badge}</span>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding:"32px 24px", maxWidth:1100, margin:"0 auto" }}>
        ...
        {activeSection === "home" && (
          <div style={{ display:"grid", gap:24 }}>
            <div style={{ display:"grid", gap:8 }}>
              ...
              <div style={{ fontSize:36, fontWeight:900, lineHeight:1.1 }}>Members Vault</div>
              <div style={{ fontSize:15, color:palette.textSoft, maxWidth:500, lineHeight:1.6 }}>Your private edge. Pattern guides, psychology reads, and tools that get better as you log more trades.</div>
            </div>

            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:16, marginTop:8 }}>
              {[
                { id:"patterns", icon:"📐", title:"Pattern Library", desc:"9 patterns. What they look like, how to trade them, and the mistakes most people make.", count:`${PATTERNS.length} patterns`, color:palette.blue, tone:"blue" },
                { id:"psychology", icon:"🧠", title:"Psychology Vault", desc:"30-second reads on the mental patterns that kill accounts. No motivation. Just the truth.", count:`${PSYCH_ARTICLES.length} articles`, color:palette.gold, tone:"gold" },
                { id:"dna", icon:"🧬", title:"Trader DNA™", desc:"AI synthesizes all your logged trades into a personal performance profile. Gets smarter every trade.", count:"Claude-powered", color:palette.gold, tone:"gold", badge:"™" },
                { id:"videos", icon:"🎬", title:"Video Library", desc:"Mark Douglas and David Paul. Embedded sessions from the traders who changed how serious people think about markets.", count:"6 videos", color:palette.blue, tone:"blue" },
                { id:"coming", icon:"🚧", title:"More Coming Soon", desc:"AI Review Archive, Advanced Analytics, Hall of Fame, Prop Firm Vault, Playbook Builder.", count:"In development", color:palette.textDim, tone:"neutral", disabled:true },
              ].map(item => (
                <button key={item.id}
                  onClick={() => !item.disabled && setActiveSection(item.id)}
                  style={{ appearance:"none", border:`1px solid ${item.disabled?"rgba(255,255,255,0.05)":palette.border}`, background:"linear-gradient(180deg,rgba(15,20,32,0.96),rgba(10,14,24,0.96))", borderRadius:20, padding:"24px 26px", cursor:item.disabled?"default":"pointer", textAlign:"left", color:palette.text, display:"grid", gap:14, opacity:item.disabled?0.5:1, transition:"all 0.2s ease" }}>
                  <div style={{ fontSize:32 }}>{item.icon}</div>
                  <div>
                    <div style={{ fontSize:18, fontWeight:900, marginBottom:6 }}>{item.title}</div>
                    <div style={{ fontSize:13, color:palette.textSoft, lineHeight:1.55 }}>{item.desc}</div>
                  </div>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <span style={{ fontSize:11, color:item.color, fontWeight:700 }}>{item.count}</span>
                    {!item.disabled && <span style={{ fontSize:12, color:palette.textDim }}>Open →</span>}
                  </div>
                </button>
              ))}
            </div>
        ...
        {activeSection === "patterns" && !selectedPattern && (
          <div style={{ display:"grid", gap:20 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", flexWrap:"wrap", gap:12 }}>
              <div>
                <div style={{ fontSize:10, color:palette.textDim, letterSpacing:3, textTransform:"uppercase", marginBottom:6 }}>Members Vault</div>
                <div style={{ fontSize:28, fontWeight:900 }}>Pattern Library</div>
                <div style={{ fontSize:13, color:palette.textSoft, marginTop:4 }}>Your actual setups. Not YouTube nonsense.</div>
              </div>
              <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                {patternTones.map(f => (
                  <button key={f} onClick={() => setPatternFilter(f)}
                    style={{ appearance:"none", border:`1px solid ${patternFilter===f?palette.text:"rgba(255,255,255,0.08)"}`, background:patternFilter===f?"rgba(255,255,255,0.1)":"transparent", color:patternFilter===f?palette.text:palette.textDim, borderRadius:8, padding:"5px 12px", cursor:"pointer", fontSize:11, fontWeight:700, textTransform:"capitalize" }}>
                    {f === "all" ? "All" : f === "short" ? "Bearish" : f === "long" ? "Bullish" : f === "gold" ? "Special" : "Other"}
                  </button>
                ))}
              </div>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))", gap:14 }}>
              {filteredPatterns.map(p => <PatternCard key={p.id} pattern={p} onClick={setSelectedPattern} />)}
            </div>
          </div>
        )}
        ...
        {activeSection === "psychology" && !selectedArticle && (
          <div style={{ display:"grid", gap:20 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", flexWrap:"wrap", gap:12 }}>
              <div>
                <div style={{ fontSize:10, color:palette.textDim, letterSpacing:3, textTransform:"uppercase", marginBottom:6 }}>Members Vault</div>
                <div style={{ fontSize:28, fontWeight:900 }}>Psychology Vault</div>
                <div style={{ fontSize:13, color:palette.textSoft, marginTop:4 }}>Tiny lessons. Not motivation. The stuff that actually costs you money.</div>
              </div>
              <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                {articleCategories.map(c => (
                  <button key={c} onClick={() => setArticleFilter(c)}
                    style={{ appearance:"none", border:`1px solid ${articleFilter===c?palette.text:"rgba(255,255,255,0.08)"}`, background:articleFilter===c?"rgba(255,255,255,0.1)":"transparent", color:articleFilter===c?palette.text:palette.textDim, borderRadius:8, padding:"5px 12px", cursor:"pointer", fontSize:11, fontWeight:700 }}>
                    {c}
                  </button>
                ))}
              </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))", gap:14 }}>
              {filteredArticles.map(a => <ArticleCard key={a.id} article={a} onClick={setSelectedArticle} />)}
            </div>
          </div>
        )}
        ...
        {activeSection === "dna" && (
          <TraderDnaPage
            dna={dna} setDna={setDna}
            loading={dnaLoading} setLoading={setDnaLoading}
            error={dnaError} setError={setDnaError}
