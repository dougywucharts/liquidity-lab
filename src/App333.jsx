...
export default function App() {
      ...
      try {
        ...
        setEvents((prev) => {
          ...
          const unique = merged
            .map((item) => ({
              ...item,
              _ts: new Date(item.timestampUtc || 0).getTime() || 0,
            }))
            .filter((item) => {
  ...
  async function saveDecision() {
    ...
    const linkedRadarEvent =
      logMode === "event" && selectedEvent ? { ...selectedEvent } : null;
        ...
        <section style={ui.split}>
                      ...
                      >
                        <div style={{ display: "grid", gap: 8, flex: 1 }}>
                          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                            ...
                            {Number(event.botConfidence || 0) >= 0.85 ? (
                              <span style={ui.redPill}>HOT</span>
                            ) : null}
                          </div>
                          ...
                          <div
                            style={{
                              fontWeight: 900,
                              fontSize: 18,
                              color: level === "L3" ? "#ffb0b0" : level === "L2" ? "#ffd38b" : "#f3f3f3",
                            }}
                          >
                            {event.sweepType}
                          </div>
          ...
          <div style={ui.card}>
              ...
              {decisionForm.screenshot ? (
                <div style={{ marginTop: 12 }}>
                  ...
                  <div style={{ marginTop: 8 }}>
                    <button style={ui.button} type="button" onClick={clearScreenshot}>
                      Clear Screenshot
                    </button>
                  </div>
                </div>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 14 }}>
              <button style={ui.primaryButton} onClick={saveDecision} type="button">
                  ...
                  <div key={item.id} style={ui.collapsedLogCard}>
                    ...
                    {expanded ? (
                      <>
                        ...
                        {item.aiSummary ? (
                          <div style={{ marginTop: 10, color: "rgba(255,255,255,0.88)", lineHeight: 1.45 }}>
                        ...
                        {item.chartRead ? (
                          <div style={{ marginTop: 10, color: "rgba(255,255,255,0.72)", lineHeight: 1.45 }}>
                            <strong style={{ display: "block", marginBottom: 6 }}>Chart Read</strong>
                            {item.chartRead}
                          </div>
                        ) : null}

                        {item.setupAssessment ? (
                          <div style={{ marginTop: 10, color: "rgba(255,255,255,0.72)", lineHeight: 1.45 }}>
                            <strong style={{ display: "block", marginBottom: 6 }}>Setup Assessment</strong>
                            {item.setupAssessment}
                          </div>
                        ) : null}

                        {item.executionAssessment ? (
                          <div style={{ marginTop: 10, color: "rgba(255,255,255,0.72)", lineHeight: 1.45 }}>
                            <strong style={{ display: "block", marginBottom: 6 }}>Execution Assessment</strong>
                            {item.executionAssessment}
                          </div>
                        ) : null}
                        ...
                        {item.riskAssessment ? (
                          <div style={{ marginTop: 10, color: "rgba(255,255,255,0.72)", lineHeight: 1.45 }}>
                            <strong style={{ display: "block", marginBottom: 6 }}>Risk Assessment</strong>
                            {item.riskAssessment}
                          </div>
                        ) : null}
                        ...
                        {tags.length ? (
                          <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap" }}>
                            {tags.map((tag, idx) => (
                              <span key={`${tag}-${idx}`} style={ui.amberPill}>{tag}</span>
                            ))}
                          </div>
                        ) : null}
                        ...
                        {good.length ? (
                          <div style={{ marginTop: 10, color: "rgba(255,255,255,0.72)", lineHeight: 1.45 }}>
                            <strong style={{ display: "block", marginBottom: 6 }}>What Was Good</strong>
                            {good.map((line, idx) => (
                        ...
                        {work.length ? (
                          <div style={{ marginTop: 10, color: "rgba(255,255,255,0.72)", lineHeight: 1.45 }}>
                            <strong style={{ display: "block", marginBottom: 6 }}>What Needs Work</strong>
                            {work.map((line, idx) => (
                              <div key={idx}>• {line}</div>
                            ))}
                          </div>
                        ...
                        {item.aiCoachingNote ? (
                          <div style={{ marginTop: 10, color: "rgba(255,255,255,0.72)", lineHeight: 1.45 }}>
                            <strong style={{ display: "block", marginBottom: 6 }}>Coaching Note</strong>
                            {item.aiCoachingNote}
                          </div>
                        ) : null}
