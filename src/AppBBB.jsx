  ...
  for (const event of events) {
    ...
    if (!existing) {
      grouped.set(key, {
        ...
        groupedConfidence: [Number(event.botConfidence || 0)],
      });
      continue;
    }
    ...
    if (shouldReplace) {
      grouped.set(key, {
        ...
        groupedConfidence: [...(existing.groupedConfidence || []), Number(event.botConfidence || 0)],
      });
    } else {
...
export default function App() {
                          ...
                          <div style={{ display: "grid", gap: 8, flex: 1 }}>
                            ...
                            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                              ...
                              {event.groupedCount > 1 ? (
                                <span style={ui.pill}>{event.groupedCount} linked</span>
                              ) : null}
                            </div>
  ...
  <input
    ...
    onChange={(e) => {
      ...
      reader.onloadend = () => {
        updateDecision("screenshot", reader.result);
      };
