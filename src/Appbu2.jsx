...
const ui = {
  ...
  selectedRing: {
    boxShadow: "0 0 0 1px rgba(255,255,255,0.16), 0 0 28px rgba(255,26,26,0.14), 0 18px 36px rgba(0,0,0,0.34)",
  },
  tableGrid: {
    display: "grid",
  ...
  statValue: { fontSize: 30, fontWeight: 900, marginTop: 8, letterSpacing: "-0.03em" },
...
function radarCardStyle(level) {
  if (level === "L3") {
    return {
      ...ui.radarItemBase,
      background: "linear-gradient(180deg, rgba(90,14,14,0.42), rgba(18,18,18,0.98))",
      border: "1px solid rgba(255,40,40,0.34)",
      boxShadow: "0 0 24px rgba(255,40,40,0.14), 0 14px 30px rgba(0,0,0,0.34)",
    };
  }
  if (level === "L2") {
    return {
      ...ui.radarItemBase,
      background: "linear-gradient(180deg, rgba(92,62,10,0.24), rgba(18,18,18,0.98))",
      border: "1px solid rgba(255,190,70,0.26)",
      boxShadow: "0 0 18px rgba(255,190,70,0.08), 0 14px 30px rgba(0,0,0,0.28)",
    };
  }
  return {
    ...ui.radarItemBase,
...
export default function App() {
  ...
  const stats = useMemo(() => {
    ...
    const winRate = loggedDecisions.length
      ? Math.round(
          (loggedDecisions.filter((d) => Number(d.pnl || 0) > 0).length / loggedDecisions.length) *
            100
        )
      : 0;
          ...
          >
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {displayLogTimestamp(event.timestampUtc)}
                          </div>
                        </div>
            ...
            {filteredEvents.length > visibleCount ? (
              <div style={{ marginTop: 12 }}>
                <button style={ui.button} type="button" onClick={() => setVisibleCount((prev) => prev + 6)}>
                  Show More
                </button>
              </div>
            ) : null}
                    ...
                    {expanded ? (
                      <>
                        ...
                        {(item.aiClassification || item.aiSummary || item.aiCoachingNote) ? (
                          ...
                          >
                            <div style={{ fontWeight: 800, marginBottom: 8 }}>AI Review</div>

                            {item.aiClassification ? (
                              <div style={{ marginBottom: 6 }}>
                                <strong>Classification:</strong> {item.aiClassification}
                              </div>
                            ) : null}
                            ...
                            {item.aiCoachingNote ? (
                              <div>
                                <strong>Coaching:</strong> {item.aiCoachingNote}
                              </div>
                            ) : null}
                          </div>
                        ) : null}
