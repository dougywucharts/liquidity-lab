...
const ui = {
  ...
  split: {
    display: "grid",
    gridTemplateColumns: "minmax(380px, 1fr) minmax(420px, 0.95fr)",
    gap: 14,
  },
  blockTitle: {
    fontSize: 12,
  ...
  selectedRing: {
    boxShadow: "0 0 0 1px rgba(255,255,255,0.14), 0 0 24px rgba(255,26,26,0.10)",
  },
  tableGrid: {
    display: "grid",
...
function radarCardStyle(level) {
  if (level === "L3") {
    return {
      ...ui.radarItemBase,
      background: "linear-gradient(180deg, rgba(85,10,10,0.35), rgba(18,18,18,0.98))",
      border: "1px solid rgba(255,26,26,0.32)",
      boxShadow: "0 0 16px rgba(255,26,26,0.10)",
    };
  }
  if (level === "L2") {
    return {
      ...ui.radarItemBase,
      background: "linear-gradient(180deg, rgba(82,58,12,0.22), rgba(18,18,18,0.98))",
...
function StatCard({ title, value, subtitle, level = null }) {
  ...
  if (level === "L3") {
    extra = {
      ...
      boxShadow: "0 0 24px rgba(255,26,26,0.14), 0 14px 34px rgba(0,0,0,0.28)",
      background: "linear-gradient(180deg, rgba(55,12,12,0.96), rgba(12,12,12,0.98))",
    };
  } else if (level === "L2") {
    extra = {
      border: "1px solid rgba(255,190,70,0.22)",
      background: "linear-gradient(180deg, rgba(50,35,8,0.9), rgba(12,12,12,0.98))",
    };
  }
  return (
...
export default function App() {
  ...
  useEffect(() => {
    localStorage.setItem(STORAGE_KEYS.loggedDecisions, JSON.stringify(loggedDecisions));
  }, [loggedDecisions]);
    ...
    async function loadEvents() {
      try {
        ...
        setEvents((prev) => {
          ...
          unique.sort((a, b) => new Date(b.timestampUtc || 0) - new Date(a.timestampUtc || 0));
  ...
  const filteredEvents = useMemo(() => {
    ...
    results.sort((a, b) => {
      const levelDiff = getLevelPriority(b.signalLevel) - getLevelPriority(a.signalLevel);
      if (levelDiff !== 0) return levelDiff;
      return new Date(b.timestampUtc || 0) - new Date(a.timestampUtc || 0);
    });
    return results;
  ...
  const visibleLoggedDecisions = useMemo(() => loggedDecisions.slice(0, visibleLogsCount), [loggedDecisions, visibleLogsCount]);
  ...
  function saveDecision() {
    ...
    const entry = {
      ...
      displayTime: logMode === "event" ? displayTime(selectedEvent?.timestampUtc) : "Manual",
      pair,
      timeframe: decisionForm.timeframe,
      session: decisionForm.session,
      directionBias: decisionForm.directionBias,
  ...
  function exportJson() {
    const exportPayload = { exportedAt: new Date().toISOString(), propAccount, stats, events, loggedDecisions };
      ...
      <div style={ui.container}>
          ...
          <div style={ui.toolbar}>
            <div>
              ...
              <div style={{ marginTop: 8, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                ...
                <span style={decisionForm.directionBias === "Long" || selectedEvent?.directionBias === "Long" ? ui.greenPill : ui.redPill}>{logMode === "event" ? selectedEvent?.directionBias || "Bias" : decisionForm.directionBias}</span>
            ...
            {visibleEvents.length === 0 ? (
              ...
              <div style={{ display: "grid", gap: 10 }}>
                {visibleEvents.map((event) => {
                  ...
                  const finalStyle = isSelected ? { ...baseStyle, ...ui.selectedRing } : baseStyle;
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "start", flexWrap: "wrap" }}>
                        <div style={{ display: "grid", gap: 8, flex: 1 }}>
                          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                            ...
                            {Number(event.botConfidence || 0) >= 0.85 ? <span style={ui.redPill}>HOT</span> : null}
                          </div>

                          <div style={{ fontWeight: 900, fontSize: 18, color: level === "L3" ? "#ffb0b0" : level === "L2" ? "#ffd38b" : "#f3f3f3" }}>
                            {event.sweepType}
                          </div>
                        ...
                        <div style={{ color: "rgba(255,255,255,0.55)", fontSize: 12, minWidth: 60, textAlign: "right" }}>
                          {displayTime(event.timestampUtc)}
                        </div>
                      </div>

                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12 }}>
                        <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.14)" }} />
                        <div style={{ color: level === "L3" ? "#ff7676" : level === "L2" ? "#ffcc7a" : "rgba(255,255,255,0.45)", fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase" }}>
                          liquidity line
                        </div>
                      </div>
                    </button>
        ...
        <section style={{ ...ui.card, ...ui.sectionSpacing }}>
              ...
              {visibleLoggedDecisions.map((item) => (
                <div key={item.id} style={{ ...ui.card, background: "#141414", padding: 14 }}>
                  ...
                  <div style={{ ...ui.tableGrid, marginTop: 12 }}>
                    ...
                    <MiniBox label="Lev" value={`${item.leverage || 1}x`} />
                  </div>
                </div>
              ))}
            </div>
          ...
          {loggedDecisions.length > visibleLogsCount ? (
            <div style={{ marginTop: 12 }}>
              <button style={ui.button} type="button" onClick={() => setVisibleLogsCount((prev) => prev + 6)}>Show More Logs</button>
            </div>
          ) : null}
        </section>
        ...
        <section style={{ ...ui.card, ...ui.sectionSpacing }}>
          <div style={ui.toolbar}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button style={ui.button} onClick={() => setShowPropAdvanced((prev) => !prev)} type="button">{showPropAdvanced ? "Hide Advanced" : "Show Advanced"}</button>
