...
function buildTickerItems(events, limit = 14) {
  const ranked = [...events]
    .map((event) => {
      ...
      const freshnessBoost = Math.max(0, 180000 - Math.max(0, Date.now() - ts)) / 180000;
      const score = getLevelWeight(level) * 100 + confidence * 100 + freshnessBoost * 10;
      return { ...event, signalLevel: level, _tickerScore: score, _tickerTs: ts };
    })
    .sort((a, b) => (b._tickerScore - a._tickerScore) || (b._tickerTs - a._tickerTs));
...
export default function App() {
  ...
  const tickerItems = useMemo(() => buildTickerItems(filteredEvents), [filteredEvents]);
                  ...
                  {[...tickerItems, ...tickerItems].map((item, idx) => {
                    ...
                    return (
                      <button
                        ...
                        style={{ ...ui.tickerItem, ...palette, cursor: "pointer" }}
                        title={`${item.pair} ${item.sweepType} ${item.directionBias} ${displayTime(item.timestampUtc)}`}
                      >
                        <span>{item.pair}</span>
                        <span style={levelPillStyle(item.signalLevel || "L1")}>{item.signalLevel || "L1"}</span>
                        <span>{item.directionBias}</span>
                        <span>{item.sweepType}</span>
