# PATCHED: feed bridge + deprecation + signal ignition adjustments
# =========================================================
...
BRIDGE_URL = "http://localhost:3001/events"
...
VOL_MULT_DETECT = 0.85
VOL_MULT_CONFIRM = 0.70
RANGE_MULT_DETECT = 0.35
RANGE_MULT_CONFIRM = 0.20
WICK_PCT_MIN = 0.28
REJECT_WICK_PCT_MIN = 0.12
...
MIN_RR_TO_ALERT = 0.35

# Cooldowns (tuned)
DETECTED_COOLDOWN = 30
RECLAIM_COOLDOWN = 30
ACCEPTED_COOLDOWN = 30
CONFIRMED_COOLDOWN = 30
...
def detect_setup_sweep(setup_df, map_df):
    ...
    if not pd.isna(vol_avg10) and vol_avg10 > 0:
        v_mult = c["volume"] / vol_avg10
        strength += min(20, max(0, (v_mult - 0.8) * 15))
...
def post_sweep_to_radar(
    ...
    try:
        ...
        payload = {
            ...
            "timestampUtc": last.name.isoformat() if hasattr(last.name, "isoformat") else datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
            ...
            "volumeSpike": None if pd.isna(last.get("volume", None)) else float(last.get("volume", 0)),
        ...
        resp = requests.post(BRIDGE_URL, json=payload, timeout=BRIDGE_TIMEOUT)

        if resp.status_code >= 300:
            print(f"[RADAR WARN] POST {BRIDGE_URL} -> {resp.status_code} {resp.text}")
        else:
            print(f"[RADAR OK] POST {BRIDGE_URL} -> {payload['pair']} {event_type}")
...
def main_loop():
    ...
    print("[RUN] Sweep Hunter v41.2 RADAR started.\n")
    for s in symbols:
        print("   ", s)
    print()
    ...
    while True:
        ...
        if cycle_num == 0:
            test_payload = {
                ...
                "directionBias": "Short",
                "botConfidence": 0.99,
                "timestampUtc": datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
            }
        ...
        for i, symbol in enumerate(symbols, 1):
            ...
            try:
                ...
                dbg(
                    f"   [SETUP CHECK] flag={sweep_flag} dir={sweep_dir} strength={strength} "
                    f"map={fmt(map_level, 4) if map_level is not None else '—'} "
                    f"setup={fmt(setup_level, 4) if setup_level is not None else '—'} "
                    f"liq={liquidity_type}"
                )
